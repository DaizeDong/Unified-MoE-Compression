...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, layer_index, mode):
        super().__init__()
        self.hidden_dim = config.hidden_size
        self.ffn_dim = config.intermediate_size
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        #####################################
        # print(f"routing_weights_sparsity: {routing_weights_sparsity}")
...
class DynamicSkippingMixtralSparseMoeBlockWrapper(nn.Module):
    def __init__(self, model, delta=0.):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # routing_weights = routing_weights - self.f(self.sparseThreshold)
        # routing_weights = routing_weights * (routing_weights > 0).bool()
        # selected_experts = selected_experts * (routing_weights > 0).bool()
        # print(f"Threshold: {(self.sparseThreshold, self.f(self.sparseThreshold))}")

        # mask = (sparseFunction(routing_weights, self.sparseThreshold, self.activation, self.f)).bool().detach()
        # routing_weights = routing_weights - (1 - self.scale) * self.f(self.sparseThreshold).detach() - self.scale * self.f(self.sparseThreshold)

        # mask = (routing_weights > self.delta).bool().detach()
        # routing_weights = routing_weights - (1 - self.scale) * self.sparseThreshold.detach() - self.scale * self.sparseThreshold
        ...
        # expert_mask = expert_mask * mask.unsqueeze(-1)
...
# class DCDMixtralSparseMoeBlock(nn.Module):
#     """
#     This implementation is
#     strictly equivalent to standard MoE with full capacity (no
#     dropped tokens). It's faster since it formulates MoE operations
