...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralConfig):
        ...
        self.w1 = Expert(self.hidden_dim, self.ffn_dim, bias=False)
        self.w2 = Expert(self.ffn_dim, self.hidden_dim, bias=False)
        self.w3 = Expert(self.hidden_dim, self.ffn_dim, bias=False)
    ...
    def forward(self, hidden_states, rounting_scores=None):
        current_hidden_states = self.act_fn(self.w1(hidden_states, rounting_scores)) * self.w3(hidden_states, rounting_scores)
        current_hidden_states = self.w2(current_hidden_states, rounting_scores)
        return current_hidden_states
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            routing_scores = routing_weights[top_x_list, idx_list, None]
            current_hidden_states = expert_layer(current_state, routing_scores) * routing_weights[top_x_list, idx_list, None]
