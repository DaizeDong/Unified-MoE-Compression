...
class Evaluator_Sparse:
    def __init__(self, args: Optional[Dict[str, Any]] = None) -> None:
        self.model_args, self.data_args, self.eval_args, finetuning_args, pruning_args = get_eval_sparse_args(args)
        ...
        if pruning_args.sparsity_ratio != 0:
            ...
            if pruning_args.prune_method == "wanda":
                prune_wanda(pruning_args, self.model, self.tokenizer, prune_n=prune_n, prune_m=prune_m)
            ...
            ################################################################
            print("*" * 30)
            sparsity_ratio = check_sparsity_from_state_dict(self.model)
